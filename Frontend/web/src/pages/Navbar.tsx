      ...
      <nav className="max-w-[85rem] mx-auto py-4 px-6 sm:flex sm:items-center sm:justify-between">
        <div className="flex items-center justify-between">
          ...
          <Link className="text-3xl font-semibold text-gray-800 dark:text-cyan-500" to="/">
            <span className="text-cyan-800">Libra</span>rium
          </Link>
          <button
            type="button"
        ...
        <div id="navbar" className={`transition-all duration-300 ${isOpen ? 'block' : 'hidden'} sm:flex sm:items-center sm:justify-end`}>
          <div className="flex flex-col gap-5 mt-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
            ...
            {isAuthenticated ? (
              <>
                {isAdmin && (
                  <Link to="/adminProfile" className="font-medium text-gray-800 dark:text-cyan-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition duration-300 ease-in-out">
                    Admin Profile
                  </Link>
                )}
                {!isAdmin && (
                  <Link to="/userProfile" className="font-medium text-gray-800 dark:text-cyan-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition duration-300 ease-in-out">
                    User Profile
                  </Link>
                )}
                <button
